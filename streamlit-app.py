...
def update_data():
    ...
    # Fetch data from Kafka on aggregated turnout by region
    region_consumer = create_kafka_consumer("aggregated_turnout_by_region")
    region_data = fetch_data_from_kafka(region_consumer)
    region_result = pd.DataFrame(region_data)

    # Identify regions with maximum turnout
    region_result = region_result.loc[
        region_result.groupby("region")["total_votes"].idxmax()
    ]
    region_result = region_result.reset_index(drop=True)

    # Display region-based voter information with pagination
    st.header("Voter Turnout by Region")
    paginate_table(region_result)

    # Fetch data from Kafka on aggregated turnout by constituency
    constituency_consumer = create_kafka_consumer("aggregated_turnout_by_constituency")
    constituency_data = fetch_data_from_kafka(constituency_consumer)
    constituency_result = pd.DataFrame(constituency_data)

    # Identify constituencies with maximum turnout
    constituency_result = constituency_result.loc[
        constituency_result.groupby("constituency")["total_votes"].idxmax()
    ]
    constituency_result = constituency_result.reset_index(drop=True)

    # Display constituency-based voter information with pagination
    st.header("Voter Turnout by Constituency")
    paginate_table(constituency_result)
...
st.title("Namibia Real-time Election Dashboard")
